...
- Without `neural_backend`, behaviour is unchanged and purely statistical — no
  torch required.
- In `"seq2seq"` / `"hybrid"` modes, the neural pass only runs when structural
  noise is detected (Latin-script tokens, ZWJ-damaged ligatures, whitespace
  artifacts) or a word cannot be fixed from the dictionary. Clean
  in-dictionary sentences never touch the model.
